...
namespace INSTINCT_LLM_NS {
    ...
    struct ScratchPadFormatOptions {
        bool include_action = true;
        bool include_thought = true;
    ...
    public:
        ...
        static void BuildAgentScratchPad(const LLMCompilerTaskGraph& graph, std::string& output, const ScratchPadFormatOptions& options = {}) {
            for (const auto& task: graph.tasks()) {
                if (task.tool_call().function().name() == "join") {
                    continue;
                }
                if (StringUtils::IsNotBlankString(task.result().content())) {
                    if (options.include_thought && StringUtils::IsNotBlankString(task.thought())) {
