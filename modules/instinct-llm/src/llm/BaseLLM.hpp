    ...
    public:
        ...
        ResultIteratorPtr<std::string> Batch(
            ...
            if (batched_result.generations_size()>0) {
                auto result_string_view = batched_result.generations() | std::views::transform(details::conv_languange_result_to_string);
        ...
        ResultIteratorPtr<std::string> Stream(const PromptValueVariant& input) override {
            const auto string_prompt = details::conv_prompt_value_variant_to_string(input);
            return create_result_itr_with_transform(details::conv_languange_result_to_string, StreamGenerate(string_prompt));
        }
