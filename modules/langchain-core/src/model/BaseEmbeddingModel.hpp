...
#include "document/Embedding.hpp"
...
public:
    ...
    virtual std::vector<Embedding> EmbedDocuments(const std::vector<std::string>& texts, const LLMRuntimeOptions& options) = 0;
...
inline std::vector<Embedding> BaseEmbeddingModel::Stream(const std::string& input, const LLMRuntimeOptions& options) {
